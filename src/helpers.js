...
const obtainIcbcAddress = async (icbcArrOne) => {
  ...
  const eventLocation = await Promise.all(icbcArrOne.map(event => (axios.get(`https://comcalc-server.herokuapp.com/client/find/icbc/${event.summary}`)
    .then(({data})=> data))
  ))
...
const obtainWsbcAddress = async (icbcArrOne) => {
  console.log("firstMap icbcArrOne", icbcArrOne);
  const eventLocation = await Promise.all(icbcArrOne.map(event => (axios.get(`https://comcalc-server.herokuapp.com/client/find/wsbc/${event.summary}`)
    .then(({data})=> data))
  ))
  // console.log("eventLocation-from-first-map",eventLocation[0].data.rows[0].address);
  return eventLocation;
...
const fetchAppointmentsIcbc = async(eventsObject) => {
  const eventLocation = await Promise.all(eventsObject.map(event => (axios.get(`https://comcalc-server.herokuapp.com/client/find/icbc/${event.summary}`)
...
const fetchAppointmentsWsbc = async(eventsObject) => {
  const eventLocation = await Promise.all(eventsObject.map(event => (axios.get(`https://comcalc-server.herokuapp.com/client/find/wsbc/${event.summary}`)
    .then((data)=> data.data.rows[0]))
  ))
  const emptyObj = {};
  emptyObj["eventsObject"] = eventsObject;
